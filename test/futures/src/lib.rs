...
fn lazy_async_add(mut cx: FunctionContext) -> JsResult<JsPromise> {
...
fn lazy_async_sum(mut cx: FunctionContext) -> JsResult<JsPromise> {
    let nums = cx
...
fn main(mut cx: ModuleContext) -> NeonResult<()> {
    cx.export_function("lazyAsyncAdd", lazy_async_add)?;
    cx.export_function("lazyAsyncSum", lazy_async_sum)?;
